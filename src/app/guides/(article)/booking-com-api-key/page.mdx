  ...
  data={{
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: 'proxy_country lets you query any hotel as if searching from any market. Set proxy_country=DE to see the rate a German shopper sees; change it to JP for the Japanese rate. This is how rate-parity monitoring works: one named hotel, a couple of markets, and each market asked several times, because a rate can move between identical requests and one reading per market cannot tell the two apart.',
        },
      },
    ],
  }}
...
**Short answer:** Booking.com's official Partner API requires an existing OTA business. For developers who need live Booking.com data without partnership requirements, subscribe to the Booking Live API on RapidAPI. You get a key in under 2 minutes with instant access, no approval wait, and proxy_country geo-pricing that the official API doesn't offer.
...
Why this matters: **rate-parity monitoring**. One named hotel, a couple of markets, each asked more than once, and you can tell a market being quoted differently from a rate that simply moved:

```bash
# German shopper -- repeat this call a few times, not just once
curl ... -d '{"destination": "Rixos Sungate, Antalya", "proxy_country": "DE", ...}'
# Japanese shopper -- same number of repeats
curl ... -d '{"destination": "Rixos Sungate, Antalya", "proxy_country": "JP", ...}'
```

Booking.com shows different rates to different markets. `proxy_country` lets you check any of them. Repeat each market a few times and treat a gap as real only when one market's whole range sits below the other's; the full method is in [How to monitor hotel rate parity](/guides/monitor-hotel-rate-parity).
