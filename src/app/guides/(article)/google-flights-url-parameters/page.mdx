...
**Short answer:** Google Flights' `tfs=` parameter is a base64url-encoded protocol-buffer
message packing origin, destination, dates, cabin, and stops into an undocumented blob
you can decode with `protoc --decode_raw` but should never try to build yourself. Every
result from FlightPowers' Google Flights Live API already carries a working `buy_link`
with the blob pre-encoded, so you never need to write a `tfs=` encoder.
...
## What are the three parameters that matter?
...
## How do you decode tfs= at the wire level?
...
you one (the `protoc` snippet above),
