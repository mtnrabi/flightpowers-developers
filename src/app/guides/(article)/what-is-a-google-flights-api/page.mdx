...
**Short answer:** A Google Flights API returns live flight prices from Google's public site as JSON. It runs a search on your behalf and gives you structured data: price, airline, stops, and Google's own low/typical/high verdict on every fare. You get the data travelers see right now, plus the historical context to decide if a price is worth booking.
...
## How does X-Search-Status distinguish empty from failed searches?
...
Or drop the `headers` block: the same URL answers a credential-less client with a sign-in, and you paste your key once on the page that opens instead of into the file.
