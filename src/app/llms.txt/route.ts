...
- ${COUNTS.mcpServers} hosted MCP servers (streamable HTTP), one URL each: ${LINKS.mcpFlights} (flights) and ${LINKS.mcpHotels} (hotels). Add the URL, the client shows a Sign in button, you sign in with Google and paste your RapidAPI key once on the connect page. Scripts, CI and clients without a sign-in button send the key on that same URL, in an x-rapidapi-key header or as ?rapidapi_key=. ${LINKS.mcpFree} serves all four tools with no RapidAPI key at all: sign in with Google, 150 searches a day and 2,000 a month per person, ad-supported
...
Flights and hotels are separate subscriptions; within each API every plan includes every endpoint, differing only on volume and rate limit. Billing is on RapidAPI. Apify actors are pay-per-event, per each actor's own event table read 2026-09-07: flights $1.85 per 1,000 lookups, hotels $3.70 per 1,000 lookups.

## Free tools (no signup)

- [Live demo](https://demo.flightpowers.com): the full consumer flight search engine, running on this same API
- [Free MCP server](${u('/tools#free-mcp')}): ${LINKS.mcpFree}, connectable from Claude or ChatGPT with no RapidAPI key. Sign in with Google once and all four tools (flights and hotels) work. Ad-supported: every successful result carries one labelled sponsored card, and the paid servers are ad-free. Caps are per signed-in person: 150 searches a day and 2,000 a calendar month, one search per date × destination combination; past the cap the tools answer search_status "rate_limited". One call searches at most 15 date × destination combinations (the paid servers do 30) and over-wide requests are sampled evenly and flagged with truncated: true. Because it carries ads it is not in any MCP directory or registry; the paid servers are.
