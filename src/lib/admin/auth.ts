 ...
 * exactly this case. So the payload is base64-decoded, not verified -- and the
 * two things that actually matter, `email_verified` and the email itself, are
...
/** Minimal id_token payload read. See the header note on why it is not verified. */
export function readIdTokenClaims(idToken: string): { email?: string; email_verified?: boolean } | null {
